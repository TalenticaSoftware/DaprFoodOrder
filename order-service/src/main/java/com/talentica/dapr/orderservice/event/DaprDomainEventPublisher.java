...
@Component
public class DaprDomainEventPublisher implements DomainEventPublisher {
    ...
    public void publish(String aggregateType, Object aggregateId, DomainEvent domainEvent) {
        CloudEvent<DomainEvent> e = new CloudEvent<>(
                UUID.randomUUID().toString(),
                "ORDER-SERVICE",
                domainEvent.getClass().getSimpleName(),
                "1.0",
                CloudEvent.CONTENT_TYPE,
                domainEvent
        );

        System.out.println("will publish the domain event");
        localProxy.create(aggregateType, e);
    ...
    public class CloudEvent<T> {
         ...
         */
        public  CloudEvent<?> deserialize(byte[] payload) throws IOException {
            if (payload == null) {
                return null;
            }
